...
static int get_ns_inode_from_pid(pid_t pid, const char *ns_name, unsigned long long *out_id)
{
    char path[PATH_MAX];
    struct stat st;
...
int load_target_info(pid_t pid, struct target_info *ti)
{
    ...
    if (get_cgroup_id_from_pid(pid, &ti->cgroup_id, ti->cgroup_path, sizeof(ti->cgroup_path)) != 0)
        return -1;
    if (get_ns_inode_from_pid(pid, "mnt", &ti->mnt_ns_id) != 0)
        return -1;
    if (get_ns_inode_from_pid(pid, "pid", &ti->pid_ns_id) != 0)
        return -1;
    if (get_ns_inode_from_pid(pid, "net", &ti->net_ns_id) != 0)
        return -1;
